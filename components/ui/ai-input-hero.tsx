...
export function HeroWave({
  ...
  useEffect(() => {
    ...
    let grainPass: ReturnType<typeof createFilmGrainPass>;
    let cameraWidth = 0,
      cameraHeight = 0,
    ...
    function updateGlowDistance() {
      ...
      (barMaterial.uniforms as Record<string, { value: number }>).uMaxGlowDist.value =
        spanPx;
    }

    function createInstancedMaterial() {
      const baseCol = new THREE.Color("hsl(250,70%,55%)");
      const emisCol = new THREE.Color("#6366f1");
    ...
    function updateGainMultiplier() {
      ...
      (barMaterial.uniforms as Record<string, { value: number }>).uGainMul.value =
    ...
    function setupPointerTracking() {
      ...
      window.addEventListener("pointerup", deactivate as EventListener, {
        passive: true,
      });
      el.addEventListener("pointerleave", deactivate as EventListener, {
        passive: true,
      });
      ...
      window.addEventListener("touchend", deactivate as EventListener, {
        passive: true,
      });
      window.addEventListener("touchcancel", deactivate as EventListener, {
        passive: true,
      });
      ...
      listeners.push(() => {
        ...
        window.removeEventListener("pointerup", deactivate as EventListener);
        el.removeEventListener("pointerleave", deactivate as EventListener);
        el.removeEventListener("touchstart", activate as EventListener);
        el.removeEventListener("touchmove", move as EventListener);
        window.removeEventListener("touchend", deactivate as EventListener);
        window.removeEventListener("touchcancel", deactivate as EventListener);
      });
    }
    ...
    function accumulateGlow(dt: number) {
      ...
      for (let i = 0; i < currentBarCount; i++) {
        ...
        const hit =
          dx < mDist ? 1.0 - Math.pow(dx / mDist, fall) : 0.0;
    ...
    function createInstancedBars() {
      ...
      geo.setAttribute(
        "aGroup",
        new THREE.InstancedBufferAttribute(aGroup, 1)
      );
      geo.setAttribute(
        "aGlow",
    ...
    function buildKeyframeTweens(
      target: typeof wave1,
      keyframes: typeof waveKeyframes1
    ) {
    ...
    function initWaveThree() {
      ...
      (waveComposer as unknown as { setPixelRatio: (n: number) => void }).setPixelRatio(
        EFFECT_PR
      );
      ...
      (
        waveBloomPass as unknown as {
          resolution: { set: (w: number, h: number) => void };
        }
      ).resolution.set(cameraWidth * 0.5, cameraHeight * 0.5);
    ...
    function applyHeavyResize() {
      ...
      (waveComposer as unknown as { setSize: (w: number, h: number) => void }).setSize(
        pendingW,
        pendingH
      );
      (
      ...
      (grainPass as unknown as { setSize?: (w: number, h: number) => void })?.setSize?.(
        pendingW,
        pendingH
      );
      (
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h1 className="text-white text-3xl sm:text-5xl font-semibold tracking-tight drop-shadow-[0_1px_8px_rgba(99,102,241,0.25)] whitespace-pre-line">
            {title}
          </h1>
            ...
            <div className="relative w-full sm:w-[720px]">
              <div className="relative rounded-2xl p-[2px] shadow-[0_1px_2px_0_rgba(0,0,0,0.06)] bg-gradient-to-br from-white/10 via-white/5 to-black/20">
                <textarea
                  ...
                  rows={5}
                  className="w-full h-32 sm:h-36 resize-none rounded-2xl bg-[rgba(15,15,20,0.55)] border border-white/10 text-white placeholder:text-white/40 outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/40 backdrop-blur-md px-4 py-4 pr-16"
                />
              </div>
              <button
                type="submit"
                aria-label={buttonText}
                className="absolute right-3 bottom-3 inline-flex items-center justify-center w-10 h-10 rounded-xl bg-primary text-white hover:bg-primary/90 transition-colors"
              >
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
